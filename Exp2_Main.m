...
Target_Position = 8;
Size_Condition = [0 1.0];
Color_Condition = [231 255 1];% hue44;sat238;lum120
Num_Condition = 3;
Distractor_Position = Target_Position - 1;
Rest_Time = 30;
Eccentricity = 4.35;% 3.4
View_Distance = 72;
...
Sub_Name = Sub_Info{2};
pwd = pwd();
if ~isdir([pwd '/Exp2_Participant_Data/' Sub_Name])
    mkdir([pwd '/Exp2_Participant_Data/' Sub_Name]);
end
Filename = ['Exp2_Result_' Sub_Number '_' Sub_Name];
...
% Screen('Preference','DefaultFontName','Times New Roman');
...
SESSION(1,:) = [zeros(1,40)  distractor_ratio distractor_ratio]; 
...
SESSION(4,:) =  [ones(1,40) ones(1,100)*2 ones(1,100)*3];% 3 conditions
...
for ntrial = 1:Actual_Ntrial
    ...
    Screen('Flip',window);
    WaitSecs(ITI);
    cd([pwd '/Exp2_Participant_Data/' Sub_Name]);
    save(Filename);
    cd ..
